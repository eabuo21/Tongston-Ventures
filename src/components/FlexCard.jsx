  ...
  return (
    <div className="flex        flex-col    relative left-[3rem]  gap-6    justify-center items-center     lg:gap-6   md:flex-row  md:gap-2">
      {flexcard1.map((card, index) => (
        <div key={index} className="w-[65] h-auto m-4 flex bg-white  hover:bg-gray-200 transition-all ease-in-out duration-700 hover:transition-all hover:border-e-transparent hover:ease-in-out hover:duration-700  ">
          <FlexNav
            image={card.image}
            text={card.text}
            link={card.link} />
